...
- `Google の審査プロセスを完了していません` という表示が出る場合は、`OAuth 同意画面` でアプリを「テストユーザー」として追加するか、審査申請を行う必要があります。
  - `外部` を選択した場合、未検証アプリでもテストユーザー登録済みユーザーは使えます
  - `内部` を選択できる場合は、同じ Google Workspace 組織のユーザーのみ利用できます
  - 審査を通すまで一般公開で使わせたい場合は、Google に審査申請を送る必要があります
